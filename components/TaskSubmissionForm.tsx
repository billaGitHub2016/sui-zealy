...
const formSchema = z.object({
  ...
  attachment: z.array(z.instanceof(File)).refine((files) => files.length > 0, "请至少上传一个图片附件。"),
})

export function TaskSubmissionForm({ onSubmitSuccess }: { onSubmitSuccess: () => void }) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <RadioGroup defaultValue={field.value?.toString()} onValueChange={(val) => { field.onChange(val) }}>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="1" id="r1" />
                    <Label htmlFor="r1">完成就送</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="2" id="r2" />
                    <Label htmlFor="r2">从完成者中抽取1位幸运儿</Label>
                  </div>
                </RadioGroup>
              </FormControl>
              <FormDescription>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                用户完成任务后申请奖励，设置申请通过数量(有多少申请可以通过)。
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}

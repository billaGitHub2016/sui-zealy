...
export async function fetchTasks(page: number, limit: number = 10): Promise<Task[]> {
      ...
      {tasks.map((task) => (
        <Link href={`/tasks/${task.id}`} key={task.id}>
          <div className="border p-4 rounded-lg shadow hover:shadow-md transition-shadow">
            ...
            <p className="text-gray-600 mb-2">{task.desc.substring(0, 100)}...</p>
            <p className="text-sm text-gray-500">发布者: {task.owner_address}</p>
